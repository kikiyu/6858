...
const char *http_request_line(int fd, char *reqpath, char *env, size_t *env_len)
{
    ...
    *sp2 = '\0';
    ...
    envp += snprintf(envp, len, "REQUEST_METHOD=%s", buf) + 1;
    envp += snprintf(envp, len, "SERVER_PROTOCOL=%s", sp2) + 1;

    /* parse out query string, e.g. "foo.py?user=bob" */
    if ((qp = strchr(sp1, '?')))
    {
        *qp = '\0';
        envp += snprintf(envp, len, "QUERY_STRING=%s", qp + 1) + 1;
    }
    ...
    envp += snprintf(envp, len, "SERVER_NAME=zoobar.org") + 1;
...
void http_serve(int fd, const char *name)
{
    ...
    strncat(pn, name, (sizeof(pn) - sizeof(name) - 1));
    split_path(pn);
